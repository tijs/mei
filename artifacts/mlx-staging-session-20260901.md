...
| `orcarouter/Qwen3.8-27B-Uncensored-MLX` → 4-bit/ | 14963e70f886455cf93090ac95bdbf4c8730cbe1 | Qwen3.8-27B-Uncensored-MLX-4bit | 4-bit affine g64 (`qwen3_5`) | staged-complete (loadability GPU-pending) |
...
`419e13a2…`); orcarouter 4-bit = 16,054,541,599 B (config_sha `f238d3a1…`,
`4-bit/` subdir relocated to model root). Provenance files:
`<target>.provenance.json` next to each staged directory. All three secondary
finalists are staged-complete; **loadability remains GPU-gated and is not
asserted here**.
